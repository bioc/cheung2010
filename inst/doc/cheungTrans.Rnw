...
if (!exists("c3t")) {
if (!file.exists("c3t.rda")) {
c3t = eqtlTests( c1s[probeId(gspl[["3"]]),], ~sex, targdir="foo", 
      runname="c1_3",
      geneApply=mclapply )
save(c3t,file="c3t.rda")
      } else load("c3t.rda") 
    }
@
...
date()
top20inds_C = topKfeats( c3t, 20, fn="inds1_3.ff", feat="geneind", ginds=inds1 )
top20scores_C = topKfeats( c3t, 20, fn="scores1_3.ff", feat="score", ginds=inds1 )
@
...
# will overwrite sco1, ind1 with the improvements available in sco2, ind2
   ...
   for (i in 1:length(snchunk)) {
      ...
      chind = t(apply(scos, 1, function(x)order(x,decreasing=TRUE)))
      sco1[i1:i2,] = rowwiseExtract( scos, chind[,1:K] )
      ind1[i1:i2,] = rowwiseExtract( ginds, chind[,1:K] )
      }
   NULL
}
...
top20scores[1:5,1:10]
top20inds[1:5,1:10]
updateKfeats( top20scores, top20scores_C, top20inds, top20inds_C )
date()
top20scores[1:5,1:10]
top20inds[1:5,1:10]
date()
