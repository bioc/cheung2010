...
c1s = getSNPlocs("ch1", as.GRanges=TRUE)
seqlevels(c1s) = gsub("ch", "chr", seqlevels(c1s))
names(c1s) = paste("rs", elementMetadata(c1s)$RefSNP_id, sep="")
c2s = getSNPlocs("ch2", as.GRanges=TRUE)
seqlevels(c2s) = gsub("ch", "chr", seqlevels(c2s))
names(c2s) = paste("rs", elementMetadata(c2s)$RefSNP_id, sep="")
