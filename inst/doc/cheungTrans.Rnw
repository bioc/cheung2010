...
%\VignetteIndexEntry{}
%\VignetteDepends{SNPlocs.Hsapiens.dbSNP.20100427, hgfocus.db, GenomicRanges, GGtools}
%\VignetteKeywords{}
%\VignettePackage{}
...
if (!exists("c17s") && file.exists("c17s.rda")) load("c17s.rda")
...
if (!exists("c19s") && file.exists("c19s.rda")) load("c19s.rda")
...
for (i in 1:22) {
 cat(i)
 cc17 = getSS("cheung2010", paste("chr", i, sep=""), probesToKeep=pr17)
 mgrsave[[i]] = eqtlTests(cc17, ~sex, targdir="cc17", runname=paste("cc17", i, sep=""))
 rm(cc17)
 gc()
 }
